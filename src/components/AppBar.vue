<template >
  <header class="hdr">
    <div class="nav-hamburger">
      <div class="container-burger" @click="showNavbar">
        <div class="line"></div>
        <div class="line"></div>
      </div>
    </div>
    <transition name="fade">
      <nav class="nav-container" v-if="showNav">
        <transition-group class="app-wrapper" name="slide-in">
          <div class="app-container" v-for="(app, i) in apps" :key="'app'+i" :style="{'--i': i}">
            <div class="app-card" :style="{'background':'linear-gradient(to bottom, '+app.colorTop+', '+app.colorBottom+')'}">
              <i :class="app.icon"></i>
            </div>
            <div class="app-title">
              {{ app.title }}
...
<style lang="scss" scoped>
  .hdr {
    background: url('../assets/images/app_bg_z.jpg');
    ...
    .slide-in-enter-active, .slide-in-leave-active {
      transition: opacity .5s linear, transform .5s cubic-bezier(0.23, 1, 0.320, 1);
